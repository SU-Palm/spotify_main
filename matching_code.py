...
def updateMatches(user): 
    ...
    if not user.id: 
        raise ValueError("Users must be in database")
...
def updateScoreMatrix(user):
    ...
    # get year from datetime value
    age = int(age_datetime.strftime("%Y"))
    ...
    for user_compared in database:
        ...
        age_datetime_compared = user_compared.get_age()
        ...
        age_compared = int(age_datetime_compared.strftime("%Y"))

        # assign values to file entries 
        entry_age = 0
        entry_gender = 0
        entry_score = 0

        if ((age_range_min > age_compared) or (age_range_max < age_compared)): 
            entry_age = '0'
            
        elif ((age_range_max >= age_compared) and (age_range_min <= age_compared)):
            entry_age = 1
        ...
        if ((score > 0) and (score < 161)):
            entry_score = char(score)

        else:
            entry_score = 0

        # write to score matrix file
            # format syntax: {[index]:[width][.precision][type]}
            # does this fill in on the left side with 0's?  
        file_score_matrix.write("{0:d}: {1:2.2d} {2:2.2d} {3:3.3d}\n".format(user_compared.id, entry_age, entry_gender, entry_score))
...
def updateMatchList(user): 
    ...
    for line in file_score_matrix:
        if ((line[-9:-8] == '-1') or (line[-6:-5] == '-1') or (line[-3:] == '-1')):
            file_unknowns_list.write(username + '/n')
        elif ((line[-9:-8] == '01') and (line[-6:-5] == '01') and (line[-3:] == '01')): 
